...
export const addsalaryslip = async (req, res) => {
  const {
   ...
   console.log(typeof(employeeId),basicSalary,bonus,deductions,allowance,grossSalary,netPay,payslipMonth,professionalTax,providentFund,securityDeposits,totalDeductions);
  try {
    ...
    const payslipDateOnly = new Date(
      ...
      payslipDate.getDate()
        ...
        if (existingSalarySlip) {
          ...
          return res.status(400).json({ error: 'Salary slip already exists for this employee and month' });
        }
